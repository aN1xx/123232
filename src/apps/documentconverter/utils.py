    ...
    for r in doc_file.part.rels.values():
        if isinstance(r._target, docx.parts.image.ImagePart):
            ...
            if img.split('.')[-1] == 'emf':
                ...
                command = 'inkscape --file {0} --export-plain-svg {1}.svg'.format(os.path.join(dir, img),
                                                                                  os.path.join(dir, img.split('.')[0]))
        ...
        # If paragraph has an image, insert an image tag with the image file
            ...
            for rId in rels:
                if rId in paragraph._p.xml:
                    text += ('\n<img style="width: 50vw;" src="' + os.path.join(MEDIA_URL, 'images/word/media',
                                                                                rels[rId]) + '">')
